...
import model.playlist.media.Song;
...
/**
 * The SongFileProcessor class is responsible for processing and validating song media files.
 * It implements the BaseFileProcessor interface to provide functionality for reading, validating,
 * and creating Song objects from file data.
 */
public class SongFileProcessor implements BaseFileProcessor {

    private FileHandler fileHandler;

    /**
     * Constructs a SongFileProcessor with the specified file handler for file operations.
     *
     * @param fileHandler The FileHandler used to read and write files.
     */
    public SongFileProcessor(FileHandler fileHandler) {
        this.fileHandler = fileHandler;
    }

    /**
     * Validates that the number of columns in the song file is valid.
     *
    ...
    /**
     * Processes the song data from the specified file path and returns a list of Media objects.
     * It validates the column size, parses the song data, and handles any formatting issues.
     *
     * @param filePath The path to the file containing song data.
     * @return A list of Media objects containing Song information.
     * @throws InvalidFormatException If the genre format is invalid.
     */
    @Override
    public List<Media> process(String filePath) throws InvalidFormatException {
        ...
        for (String playlistLine : playlistLines) {
            ...
            try {
                validateColumnSize(playListItems.length, "Song");
            ...
            SongGenre genre;
            try {
                genre = SongGenre.getFromFileLine(playListItems[Constants.FILE_COLUMN_FOUR].toUpperCase());
            ...
            int durationInMins;
            try {
                durationInMins = Integer.parseInt(playListItems[Constants.FILE_COLUMN_FIVE]);
            } catch (NumberFormatException e) {
                System.out.println(Messages.INVALID_DURATION);
                continue;
            }

            String captionFileName = playListItems[Constants.FILE_COLUMN_SIX];
            Media media = new Song(name, description, artistName, genre, durationInMins, captionFileName);
